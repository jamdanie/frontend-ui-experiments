export function initLiquidCursor() {
  ...
  function getSupportedFormat(glRef, internalFormat, format, type) {
    if (!supportRenderTextureFormat(glRef, internalFormat, format, type)) {
      switch (internalFormat) {
        case glRef.R16F:
          return getSupportedFormat(glRef, glRef.RG16F, glRef.RG, type);
        case glRef.RG16F:
          return getSupportedFormat(glRef, glRef.RGBA16F, glRef.RGBA, type);
        default:
          return null;
      }
    }
  ...
  function supportRenderTextureFormat(glRef, internalFormat, format, type) {
    const texture = glRef.createTexture();
    glRef.bindTexture(glRef.TEXTURE_2D, texture);
    glRef.texParameteri(glRef.TEXTURE_2D, glRef.TEXTURE_MIN_FILTER, glRef.NEAREST);
    glRef.texParameteri(glRef.TEXTURE_2D, glRef.TEXTURE_MAG_FILTER, glRef.NEAREST);
    ...
    const fbo = glRef.createFramebuffer();
    glRef.bindFramebuffer(glRef.FRAMEBUFFER, fbo);
    glRef.framebufferTexture2D(
      glRef.FRAMEBUFFER,
      glRef.COLOR_ATTACHMENT0,
      glRef.TEXTURE_2D,
      texture,
      0
    );

    return glRef.checkFramebufferStatus(glRef.FRAMEBUFFER) === glRef.FRAMEBUFFER_COMPLETE;
  }
